...
namespace cloud.charging.open.EMSP
{
    ...
    /// <summary>
    /// The OCPI side of this EMSP: the endpoints the roaming partners call,
    ...
    /// </para>
    /// <para>
    /// <b>What is per version.</b> The library keeps roaming partners, tokens
    ...
    /// configuration.json but who this EMSP is and which versions it offers.
    /// </para>
    /// </remarks>
    public partial class EMSP
    {
        ...
        /// <summary>
        /// Who this EMSP is in OCPI, and which versions it offers - as it was
        ...
        /// <summary>
        /// The OCPI versions this EMSP speaks, oldest first.
        /// </summary>
        public IReadOnlyList<OCPIVersion>   OCPIVersions
            => ocpiVersions;

        /// <summary>
        /// The country code and party identification of this EMSP.
        /// </summary>
        public Party_Idv3                   PartyId          { get; private set; }

        /// <summary>
        /// The same, written the way an EMSP's is written: "DE-GDF".
        /// </summary>
        /// <remarks>
        /// <see cref="Party_Idv3"/> writes itself without a separator, and
        /// with one only when told which role it is for - a CPO's is written
        /// with an asterisk, an EMSP's with a hyphen. Everything this EMSP
        /// shows about itself uses this form.
        /// </remarks>
        public String                       PartyIdText
            => PartyId.ToString(Role.EMSP);

        /// <summary>
        /// The business details of this EMSP, as its partners see them in the
        /// credentials.
        /// </summary>
        public BusinessDetails              BusinessDetails  { get; private set; } = default!;

        /// <summary>
        /// Where the OCPI endpoints of this EMSP are, from outside: the HTTPExt
        ...
        /// <summary>
        /// How many tokens this EMSP has issued, over every version.
        /// </summary>
        ...
        private void BuildOCPI(OCPIConfiguration? Configuration)
        {
            ...
            // The external URL when the operator wrote one down - an EMSP
            // behind a reverse proxy is reached by a name this process has
            // never heard of - and the address this EMSP listens on otherwise.
            ...
            ocpiAPI = new CommonHTTPAPI(
                          ...
                          Description:               I18NString.Create("The OCPI endpoints of this EMSP"),
                          ...
                          // What a CPO pushed into this EMSP is the CPO's,
            ...
            foreach (var version in ocpiSettings.EffectiveVersions)
            {
                ...
                ocpiVersions.Add(
                    version switch {
                        ...
                        _        => throw new InvalidOperationException($"'{version}' is not an OCPI version this EMSP speaks.")
                    }
                );
            ...
            #region Somebody discovering this EMSP
                    ...
                    Log.Info(
                        request.RemoteParty is not null
                            ...
                            : $"Somebody at {request.HTTPRequest.RemoteSocket} asked for the OCPI versions" +
                              (request.AccessToken.HasValue ? " with a token this EMSP does not know." : " without a token."),
                        "ocpi", "versions"
                    );
            ...
            Log.Info(
                $"OCPI: this EMSP is {PartyIdText} '{BusinessDetails.Name}', speaking {String.Join(", ", ocpiVersions.Select(version => version.Label))}; " +
        ...
        /// <summary>
        /// The OCPI side of this EMSP, as its page in the web interface reads
        ...
        public JObject OCPIConfigurationJSON()
            ...
            => new (
                   ...
                   new JProperty("counts",       new JObject(
                       ...
                       new JProperty("locations",  ocpiVersions.Sum(version => version.Locations.Count())),
        ...
        /// <summary>
        /// The version written as "2.2.1", or false when this EMSP does not
        /// offer it.
        /// </summary>
        ...
        public JObject RemotePartiesJSON(Boolean IncludeSecrets)
            ...
            => new (
                   ...
                   new JProperty("versions",   new JArray(ocpiVersions.Select(version => version.Label))),
                   new JProperty("roles",      new JArray("CPO", "EMSP", "HUB", "NSP", "SCSP", "OTHER")),
        ...
        public async Task<OCPIOperationResult> AddRemotePartyAsync(JObject JSON)
        {
            ...
            if (!TryGetOCPIVersion(label, out var version))
                return OCPIOperationResult.Failed($"'{label}' is not an OCPI version this EMSP offers; there are {String.Join(", ", ocpiVersions.Select(v => v.Label))}.");
            ...
            if (countryCode == PartyId.CountryCode && partyId == PartyId.PartyId && role == Role.EMSP)
                return OCPIOperationResult.Failed("That is this EMSP itself; an EMSP cannot be its own roaming partner.");
            ...
            if (!String.IsNullOrEmpty(theirTokenText) || !String.IsNullOrEmpty(theirURLText))
            {
                ...
                if (String.IsNullOrEmpty(theirTokenText) || String.IsNullOrEmpty(theirURLText))
                    return OCPIOperationResult.Failed("To start the peering from here both are needed: the partner's token ('theirToken') and its versions URL ('versionsURL'). Leave both empty to let the partner come to this EMSP.");
            ...
            Log.Notice(
                ...
                (spec.CanRegister
                     ? $", with its token and its versions URL {theirVersionsURL}: this EMSP can start the peering."
        ...
        /// token they handed out, and POST this EMSP's credentials to them.
        /// </summary>
        public async Task<OCPIOperationResult> RegisterRemotePartyAsync(String? Label, String? Id)
        {

            if (!TryGetOCPIVersion(Label, out var version))
                return OCPIOperationResult.Failed($"'{Label}' is not an OCPI version this EMSP offers.");
        ...
        /// <summary>
        /// Forget a roaming partner. Its token stops opening this EMSP the
        ...
        /// <summary>
        /// Every token this EMSP issued, over every version, as the Tokens
        /// page reads them.
        /// </summary>
        public JObject TokensJSON()

            => new (
                   new JProperty("tokens",     new JArray(ocpiVersions.SelectMany(version => version.Tokens))),
                   new JProperty("versions",   new JArray(ocpiVersions.Select(version => version.Label))),
                   new JProperty("types",      new JArray("RFID", "APP_USER", "AD_HOC_USER", "OTHER")),
                   new JProperty("whitelists", new JArray("ALWAYS", "ALLOWED", "ALLOWED_OFFLINE", "NEVER")),
        ...
        #region AddTokenAsync(JSON)

        /// <summary>
        /// Issue a token to a customer, on the version the request names.
        /// </summary>
        public async Task<OCPIOperationResult> AddTokenAsync(JObject JSON)
        {
            ...
            if (!Token_Id.TryParse(JSON.Value<String>("uid")?.Trim() ?? "", out var tokenId))
                return OCPIOperationResult.Failed("A 'uid' is required: what the card or the app sends.");

            if (version.HasToken(tokenId))
                return OCPIOperationResult.Failed($"There already is a token '{tokenId}' on OCPI {version.Label}. Remove it first to issue it again.");

            var typeText = JSON.Value<String>("type")?.Trim().ToUpperInvariant();

            if (String.IsNullOrEmpty(typeText))
                typeText = "RFID";

            if (!TokenType.TryParse(typeText, out var tokenType))
                return OCPIOperationResult.Failed($"'{typeText}' is not a token type; there are RFID, APP_USER, AD_HOC_USER and OTHER.");

            var contractId = JSON.Value<String>("contractId")?.Trim();

            if (String.IsNullOrEmpty(contractId))
                contractId = $"{PartyId.CountryCode}-{PartyId.PartyId}-C{tokenId}";

            var whitelistText = JSON.Value<String>("whitelist")?.Trim().ToUpperInvariant();

            if (String.IsNullOrEmpty(whitelistText))
                whitelistText = "ALLOWED";

            if (!WhitelistType.TryParse(whitelistText, out var whitelist))
                return OCPIOperationResult.Failed($"'{whitelistText}' is not a whitelist type; there are ALWAYS, ALLOWED, ALLOWED_OFFLINE and NEVER.");

            Languages? language = null;

            if (JSON.Value<String>("language")?.Trim() is { Length: > 0 } languageText)
            {

                if (!Enum.TryParse<Languages>(languageText, true, out var parsedLanguage))
                    return OCPIOperationResult.Failed($"'{languageText}' is not a language this EMSP knows; use a two-letter code such as \"de\" or \"en\".");
            ...
            var spec = new TokenSpec(
                           ...
                           tokenType,
                           contractId,
                           JSON.Value<String>("issuer")?.Trim() is { Length: > 0 } issuer ? issuer : BusinessDetails.Name,
                           JSON.Value<Boolean?>("valid") ?? true,
                           whitelist,
                           JSON.Value<String>("visualNumber")?.Trim() is { Length: > 0 } visual ? visual : null,
                           language
                       );

            var error = await version.AddToken(spec);

            if (error is not null)
                return OCPIOperationResult.Failed(error);

            Log.Notice($"The token '{tokenId}' ({tokenType}, contract {contractId}) was issued on OCPI {version.Label}.", "ocpi", "tokens");

            return OCPIOperationResult.Ok(
                       $"The token '{tokenId}' was issued on OCPI {version.Label}.",
                       new JObject(
                           new JProperty("uid",      tokenId.ToString()),
                           new JProperty("version",  version.Label)
                       )
                   );
        ...
        #region RemoveTokenAsync(Label, Id)

        /// <summary>
        /// Take a token away from a customer.
        /// </summary>
        public async Task<OCPIOperationResult> RemoveTokenAsync(String? Label, String? Id)
        {

            if (!TryGetOCPIVersion(Label, out var version))
                return OCPIOperationResult.Failed($"'{Label}' is not an OCPI version this EMSP offers.");

            if (!Token_Id.TryParse(Id ?? "", out var tokenId))
                return OCPIOperationResult.Failed($"'{Id}' is not a token identification.");

            if (!version.HasToken(tokenId))
                return OCPIOperationResult.Failed($"There is no token '{tokenId}' on OCPI {version.Label}.");

            if (!await version.RemoveToken(tokenId))
                return OCPIOperationResult.Failed($"The token '{tokenId}' could not be removed.");

            Log.Notice($"The token '{tokenId}' was taken away on OCPI {version.Label}.", "ocpi", "tokens");

            return OCPIOperationResult.Ok($"The token '{tokenId}' was removed.");
        ...
        /// <summary>
        /// Everything the partners pushed of one kind, over every version.
        /// </summary>
        public JObject RoamingDataJSON(String Kind)
        {

            var items = Kind switch {
                            "locations"  => ocpiVersions.SelectMany(version => version.Locations),
                            ...
                            _            => throw new ArgumentException($"'{Kind}' is not something a roaming partner sends.", nameof(Kind))
                        };
