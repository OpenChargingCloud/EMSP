...
namespace cloud.charging.open.EMSP
{
    ...
    /// </remarks>
    public partial class EMSP : IAsyncDisposable
    {
        ...
        /// out.
        /// </remarks>
        public static readonly IPPort DefaultHTTPPort = IPPort.Parse(2355);
        ...
        /// share this server: the JSON API of this EMSP at "/api", the web
        ...
        /// <summary>
        /// How many of those servers this EMSP was told must answer: by the
        ...
        /// <summary>
        /// Serialises changes to what this EMSP is made of, so that two
        /// browsers saving at the same moment do not build half an EMSP each.
        /// </summary>
        ...
        /// <summary>
        /// Everything that happens inside this EMSP.
        /// </summary>
        ...
        /// <summary>
        /// Where everything this EMSP can be told in writing lives between
        /// starts: its name resolution, its time source, its OCPI identity.
        /// </summary>
        public EMSPConfigFile         ConfigFile             { get; }
        ...
        /// <summary>
        /// Where this EMSP reads the time.
        /// </summary>
        ...
        /// <summary>
        /// Whether this EMSP may ask its time server.
        /// </summary>
        public Boolean                NTSEnabled             { get; private set; } = true;

        /// <summary>
        /// The password this EMSP made up because there was no login file, or
        ...
        /// this EMSP is administered with - and what the OCPI endpoints hang
        /// off.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Whether those accounts are this EMSP's own, or somebody else's that
        /// it was handed.
        /// </summary>
        public Boolean                OwnsExtAPI             { get; }

        /// <summary>
        /// Whether the HTTP server is this EMSP's own, or one it was handed
        /// and shares with somebody else.
        /// </summary>
        public Boolean                OwnsHTTPServer         { get; }

        /// <summary>
        /// Everything of this EMSP - its web interface, its JSON API and,
        ...
        /// root, and "/EMSP" or the like below one.
        /// </summary>
        ...
        /// see EMSP.Clock.cs for why it is handed in rather than reached for.
        ...
        /// <summary>
        /// When this EMSP was created, by its own clock.
        /// </summary>
        ...
        /// <param name="HTTPExtAPIPath">Where this EMSP's own HTTPExt API sits below <paramref name="BasePath"/>, "/ext" by default. Ignored when one is handed in.</param>
        ...
        /// <param name="ConfigFile">Where everything this EMSP can be told in writing lives; "configuration.json" beside the process by default.</param>
        ...
        public EMSP(DNSClient?             DNSClient               = null,
                    ...
                    EMSPConfigFile?        ConfigFile              = null,
                    ...
                    TimeProvider?          TimeProvider            = null)
        {
            ...
            // the EMSP it belongs to cannot be held against anything.
            ...
            this.Version      = typeof(EMSP).Assembly.GetName().Version?.ToString(3) ?? "0.0.0";
            ...
            this.Log.Notice($"EMSP v{this.Version} starting up.", "emsp");
            ...
            this.ConfigFile = ConfigFile ?? new EMSPConfigFile(EMSPConfigFile.DefaultFileName);

            EMSPConfiguration? configuration = null;

            if (this.ConfigFile.Exists)
            {

                // A file that is there but cannot be read is not something to
                // paper over with defaults: somebody wrote down what their EMSP
            ...
            // The four this EMSP asks when nobody says otherwise - but only
            ...
            this.httpRootPath  = HTTPRootPath ?? this.BasePath + EMSPHTTPAPI.DefaultAPIPath;
            ...
            this.ExtAPI        = ExtAPI ?? new HTTPExtAPI(
                                     ...
                                     APIRobotEMailAddress:   EMailAddress.Parse("OpenChargingCloud EMSP Robot <robot@charging.cloud>"),
                                     APIRobotGPGPassphrase:  "",

                                     // Nothing here sends mail. An EMSP that
                                     ...
                                     // over plain HTTP, and an EMSP on a bench
                                     // is reached over plain HTTP.
                                     UseSecureCookies:       false,

                                     // The shortest name a role of this EMSP
            ...
            this.Frontend      = Frontend ?? new EmbeddedContentSource(HTTPRoot, typeof(EMSP).Assembly);
            ...
            if (this.Frontend.TryGet(IndexFile, out _))
            {
                ...
                this.Log.Error(
                    ...
                    "Build the frontend (npm run build in Frontend/) or point the EMSP at a directory with --frontend.",
                    "web"
                );
        ...
        public async Task Stop()
        {
            ...
            Log.Notice("The EMSP is shutting down.", "emsp");
        ...
        /// they are this EMSP's vocabulary and not somebody's data: a group
        ...
        private async Task EnsureAccounts()
        {
            ...
            // EMSP that skipped this would find no accounts at every start,
            ...
            if (firstStart)
            {
                ...
                if (organization is not Organization emspOrganization)
                    throw new InvalidOperationException("The organization of this EMSP could not be created, and an account outside one cannot sign in.");
                ...
                if (admin is null)
                    throw new InvalidOperationException("The account of this EMSP could not be created, so nobody could sign in to it.");
            ...
            foreach (var role in UserRole.All)
            {
                ...
                if (added.Result != CommandResult.Success)
                    throw new InvalidOperationException(
                              $"The user group '{role.GroupId}' of this EMSP could not be made: " +
            ...
            if (admin is not null)
            {
                ...
                {
                    throw new InvalidOperationException(
                              $"The account of this EMSP could not be put in the {UserRole.SystemAdmin.Name} group, " +
        ...
        public JObject ConfigurationJSON()
            ...
            => new (
                   ...
                   new JProperty("EMSP", new JObject(
        ...
        /// <summary>
        /// Let somebody else decide when this EMSP's log may write on the
        /// console, because they are writing on it too.
        /// </summary>
        /// <remarks>
        /// An EMSP at a console assumes the console is its own and writes an
        ...
        ///
        /// Has no effect on an EMSP whose log does not reach the console.
        /// </remarks>
