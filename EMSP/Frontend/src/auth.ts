 ...
 * and on the EMSP; this is a cache of /auth/me so that the router's guard
...
class AuthState {
    ...
    /** Ask the EMSP who we are; null when nobody is signed in. */
     ...
     *
     * Used to grey a control out, never to decide anything: the EMSP checks
...
// elsewhere, the EMSP restarted): forget the user, the guards do the rest.
onUnauthorized(() => {
    if (auth.user !== null)
        auth.set(null);
});
