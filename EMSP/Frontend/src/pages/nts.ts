...
/**
 * What the NTS client allows itself when the EMSP has not been told.
 *
 * The EMSP's answer carries the timeout it was configured with, and null where
...
/**
 * Where this EMSP reads the time: its time servers, the rules for believing
 ...
 * told the EMSP a lone host name, which the EMSP read as a group of one: four
 ...
 *
 * Every change takes effect at once, and the EMSP is told the whole list each
 * time - which is why the list is only ever changed by exactly one server at a
 * time, from a dialog, and why a change the EMSP refuses leaves the page as it
 * was.
 *
...
export const ntsPage: Page = {
    ...
    render({ root }) {
        ...
        const content = shell(root, {
            ...
            title:     'NTS client',
            subtitle:  'Where this EMSP reads the time, and how it knows the answer is real.',
        ...
        /** The whole page, from what the EMSP last said. */
        function draw(): void {
            ...
            render(content, html`
                ...
                ${mayChange ? '' : html`
                    <div class="notice">
                        ...
                        servers but not change them. That needs the EMSP or the system administrator role.
                    </div>
                `}
         ...
         *
         * Every word of the verdict comes from the EMSP: whether a time may be
         ...
         * sooner or later say "legal" where the EMSP says nothing of the kind.
         */
            ...
            render(card, html`
                ...
                <div class="kv-list">
                    ...
                    <div class="kv">
                        ...
                        <span class="v">${formatValue(now.now)} <span class="muted small">(this EMSP's own clock)</span></span>
                    </div>
                ...
                <p class="hint">
                    ${now.legal
                          ? html`
                            ...
                            `
                          : html`
                              Unverified: ${why[now.why ?? ''] ?? 'the EMSP did not say why'}. The time
                              above is still this EMSP's own clock and is still what everything below it
        ...
        /** Whether this EMSP asks anybody at all. */
        function drawSwitch(): void {
            ...
            render(must<HTMLElement>(content, '#nts-switch'), html`
                ...
                <p class="hint">
                    Switched off, this EMSP asks its time servers nothing at all - "Sync now" and the
         ...
         * last one the EMSP remembers - and none at all while the next one is
                    ...
                    <span class="hint">
                        ${mayTest
                              ? html`
                                    ...
                                    into the log. The clock of this EMSP is not stepped by it.
                                `
                              : html`Asking the servers needs the EMSP or the system administrator role.`}
                    </span>
                </div>
         ...
         * there are. And the EMSP is told the whole list when this is saved,
        ...
        function editServer(index: number | null): void {
            ...
            /**
             * Tell the EMSP the list with the one change in it, and close only
         ...
         *              EMSP writes it into the log as it was sent.
         */
        ...
        async function switchTo(on: boolean): Promise<void> {
            ...
            {
                // Back to what the EMSP has, which is not what the box says
        ...
        /** Tell the EMSP what the group is held to. */
...
/**
 * Milliseconds the way the EMSP's log writes them: one place after a point,
