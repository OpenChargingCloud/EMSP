/**
 * What the NTS page tells the EMSP when a time server is added, edited or
 * deleted, asked directly.
 *
 * Run with `npm test`. The EMSP is sent the whole list every time, so what
...
describe('the list a change sends', () => {
    ...
    it('leaves the list it was made from alone, which is what the page goes back to when the EMSP says no', () => {
