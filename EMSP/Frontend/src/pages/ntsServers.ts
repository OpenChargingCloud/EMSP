 ...
 *
 * Apart from the page, because this is the part that decides what the EMSP
 * is told - and the EMSP is told the whole list every time, so a mistake
...
/**
 * A name as somebody reads it: without the root's dot. The EMSP hands its
...
/**
 * A server as the EMSP shows it, turned back into what its configuration
 * says - with everything that is the usual left out.
 *
 * Left out rather than repeated, because the EMSP writes back what it is
 ...
 * is no place given.
 *
 * A new list rather than the old one changed: the old one is what the EMSP
 * still has, and it is what the page has to go back to when the EMSP says
 * no.
 */
