...
namespace cloud.charging.open.EMSP
{
    ...
    /// the configuration of this EMSP, its log, and one
    ...
    /// </remarks>
    public partial class EMSPHTTPAPI : HTTPAPI
    {
        ...
        /// EMSP cannot tell it from an EMSP that has gone: nginx gives up on an
        ...
        /// <summary>
        /// Cancelled when this EMSP is shutting down, so that the
        /// event streams end.
        /// </summary>
        /// <remarks>
        ...
        /// membership carries this EMSP's roles.
        /// </summary>
        ...
        public EMSPHTTPAPI(HTTPServer  HTTPServer,
            ...
            : base(HTTPServer,
                   ...
                   Description:  I18NString.Create("The JSON API of this EMSP"))

        {
            ...
            this.ExtAPI      = ExtAPI;
            this.Log         = Log;
            this.startedAt   = EMSP.TimeProvider.GetUtcNow();

            this.Version     = Version
                                   ?? typeof(EMSPHTTPAPI).Assembly.GetName().Version?.ToString(3)
                                   ?? "0.0.0";
        ...
        /// <summary>
        /// GET /api/v1/status: how this EMSP is doing right now.
        /// </summary>
        private Task<HTTPResponse> GetStatus(HTTPRequest Request)
        {
            ...
            var now = EMSP.TimeProvider.GetUtcNow();
            ...
            return Task.FromResult(
                       JSONResponse(
                           ...
                           HTTPStatusCode.OK,
                           new JObject(
                               new JProperty("service",    "EMSP"),
                               new JProperty("version",    Version),
                               new JProperty("partyId",    EMSP.PartyIdText),
        ...
        /// <summary>
        /// GET /api/v1/configuration: what this EMSP is made of.
        /// </summary>
        private Task<HTTPResponse> GetConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, EMSP.ConfigurationJSON())
                   );
        ...
        /// <summary>
        /// GET /api/v1/configuration/dns: how this EMSP resolves names.
        /// </summary>
        private Task<HTTPResponse> GetDNSConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, EMSP.DNSConfigurationJSON())
                   );
        ...
        private Task<HTTPResponse> PutDNSConfiguration(HTTPRequest Request)
        {
            ...
            if (!EMSP.TryUpdateDNSConfiguration(json, out var error))
        ...
        /// <summary>
        /// GET /api/v1/configuration/nts: where this EMSP gets the time from.
        /// </summary>
        private Task<HTTPResponse> GetNTSConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, EMSP.NTSConfigurationJSON())
                   );
        ...
        private Task<HTTPResponse> PutNTSConfiguration(HTTPRequest Request)
        {
            ...
            if (!EMSP.TryUpdateNTSConfiguration(json, out var error))
                return Task.FromResult(ErrorJSON(Request, HTTPStatusCode.BadRequest, error));
        ...
        private async Task<HTTPResponse> PostNTSSync(HTTPRequest Request)
        {
            ...
            var result = await EMSP.SyncTimeAsync(Request.CancellationToken);

            var json   = EMSP.NTSConfigurationJSON();
        ...
        private Task<HTTPResponse> GetClock(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, EMSP.ClockJSON())
                   );
        ...
        /// </summary>
        /// <remarks>
        /// Two things in here are for a proxy in front of the EMSP, and both
        /// were learned from nginx as it comes, in front of the vehicle whose
        /// stream this is (EV 1d46e21).
        ///
        ...
        /// not even its header - until a buffer is full or the EMSP has been
        /// silent long enough for nginx to give up on it. The browser never sees
        /// the stream open, so the Logs page says "reconnecting ..." and does not
            ...
            return Task.FromResult(
                       new HTTPResponse.Builder(Request) {
                           ...
                           HTTPSSEWorker   = async (response, stream) => {
                               ...
                               // EMSP shutting down ends the stream. The
                               ...
                               try
                               {
                                   ...
                                   // EMSP the browser would otherwise wait
        ...
        /// </summary>
        /// <remarks>
        /// Called by <see cref="EMSP.Stop"/> before the server is
        ...
        /// this, an EMSP with one browser on its Logs page never finishes
        /// shutting down.
        ///
        ...
        /// them find out by being refused. They are a copy of what the EMSP
