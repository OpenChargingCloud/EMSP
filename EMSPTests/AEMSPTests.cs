...
namespace cloud.charging.open.EMSP.Tests
{
    ...
    /// <summary>
    /// One EMSP, listening, for the length of one test.
    /// </summary>
    /// <remarks>
    /// A whole EMSP per test rather than one for the fixture, because
    /// half of what is worth testing here changes the EMSP: a test that
    ...
    /// </remarks>
    public abstract class AEMSPTests
    {

        #region Properties

        /// <summary>
        /// The EMSP under test, listening, from SetUp until TearDown.
        /// </summary>
        protected EMSP  EMSP   { get; private set; } = default!;

        /// <summary>
        /// The password this EMSP made up for itself at its first start.
        /// </summary>
        ...
        #region What this EMSP is made of

        /// <summary>
        /// What its configuration file says before it is built.
        /// </summary>
        /// <remarks>
        /// Overridden by a fixture that needs an EMSP with something on
        ...
        public async Task StartTheEMSP()
        {
            ...
            Password    = EMSP.GeneratedPassword
                              ?? throw new InvalidOperationException("The EMSP did not make up a password for its first start!");
        ...
        /// <summary>
        /// A browser that has signed in with the password this EMSP made
        ...
        protected async Task<HttpClient> SignedIn()
            ...
            => await SignedInAs(EMSP.DefaultAdminUser, Password);
