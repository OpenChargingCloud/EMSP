...
namespace cloud.charging.open.EMSP.Tests
{
    ...
    /// <summary>
    /// Who may ask this EMSP anything, and what happens to
    /// everybody else.
    /// </summary>
    public class AuthenticationTests : AEMSPTests
    {
        ...
        public async Task TheJSONAPIHasNoSignIn()
        {
            ...
            var response = await http.PostAsync(
                                     "/api/v1/auth/login",
                                     LoginBody(EMSP.DefaultAdminUser, Password)
                                 );
        ...
        public async Task AWrongPasswordIsRefused()
        {
            ...
            var response = await http.PostAsync(
                                     SignInPath,
                                     LoginBody(EMSP.DefaultAdminUser, "not the password")
                                 );
        ...
        /// <summary>
        /// The password this EMSP made up at its first start, shown once
        ...
        public async Task TheGeneratedPasswordSignsIn()
        {
            ...
            Assert.That(me.Value<String>("username"), Is.EqualTo(EMSP.DefaultAdminUser));
        ...
        public async Task TheAccountSurvivesARestart()
        {
            ...
            // handle for the second EMSP to trip over.
            await EMSP.Stop();
            ...
            try
            {
                ...
                var       signIn    = await http.PostAsync(SignInPath, LoginBody(EMSP.DefaultAdminUser, Password));

                Assert.That(signIn.IsSuccessStatusCode, Is.True,
                            "The password from the first start no longer opens the EMSP.");
        ...
        /// copy of what the EMSP enforces and not the enforcement itself;
        /// this is the copy.
        /// </summary>
        [Test]
        ...
        public async Task SigningOutEndsTheSession()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(EMSP.ExtAPI.Sessions.Count(), Is.EqualTo(0),
            ...
            Assert.That((await http.GetAsync("/api/v1/auth/me")).StatusCode,
                        Is.EqualTo(HttpStatusCode.Unauthorized),
                        "The cookie still opened the EMSP after signing out.");
        ...
        public async Task SigningOutExpiresTheCookieIn1970()
        {
            ...
            var cookie  = logout.Headers.TryGetValues("Set-Cookie", out var values)
                              ? values.FirstOrDefault(value => value.StartsWith(EMSP.ExtAPI.SessionCookieName.ToString(), StringComparison.Ordinal))
                              : null;
        ...
        public async Task ACrossSiteChangeIsRefused()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(EMSP.DNSClient.UseCache, Is.True,
        ...
        /// <summary>
        /// The other half of the test above: the header that the EMSP
