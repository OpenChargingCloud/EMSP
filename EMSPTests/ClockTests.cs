...
namespace cloud.charging.open.EMSP.Tests
{
    ...
    /// </summary>
    /// <remarks>
    /// The word this EMSP must never guess is "legal", and these are the
    /// tests of it not guessing. Each builds an EMSP and does not start
    /// it: <c>ClockJSON()</c> is the whole subject, no socket is needed for it,
    /// and an unstarted EMSP is also one that has not scheduled a check
    ...
    public class ClockTests
    {
        ...
        {
            directory = TestEMSPs.TemporaryDirectory("clock");
        }
        ...
        /// whether that is any good. The first is always this EMSP's own
        ...
        public async Task AClaimedAuthorityAloneIsStillNotLegalTime()
        {
            ...
            await using var EMSP = TestEMSPs.New(
                                             directory,
                                             new JObject(
            ...
            Assert.Multiple(() => {
                Assert.That(EMSP.LegalTimeAuthority,     Is.EqualTo("PTB"));
        ...
        public async Task TheToleranceAndTheMaxAgeAreReported()
        {
            ...
            Assert.Multiple(() => {
                Assert.That(EMSP.LegalTimeTolerance,          Is.EqualTo(TimeSpan.FromSeconds(0.5)));
                Assert.That(EMSP.LegalTimeMaxAge,             Is.EqualTo(TimeSpan.FromMinutes(15)));
