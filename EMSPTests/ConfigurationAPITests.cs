...
namespace cloud.charging.open.EMSP.Tests
{
    ...
    /// <summary>
    /// What this EMSP says it is, and what happens when somebody
    ...
    public class ConfigurationAPITests : AEMSPTests
    {
        ...
        public async Task TheStatusResourceSaysWhatIsRunning()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(status.Value<String>("version"),  Is.EqualTo(EMSP.Version));
        ...
        public async Task TheConfigurationNamesEverySection()
        {
            ...
            Assert.Multiple(() => {
                Assert.That(configuration["EMSP"], Is.Not.Null);
                ...
                Assert.That(configuration["ocpi"],       Is.Not.Null);
        ...
        public async Task TheDNSConfigurationIsReadable()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(dns.Value<String>("file"),       Is.EqualTo(EMSP.ConfigFile.Path));
            });
        ...
        public async Task ChangingTheDNSSettingsReachesTheClientAndTheFile()
        {
            ...
            var onDisk   = JObject.Parse(File.ReadAllText(EMSP.ConfigFile.Path));

            Assert.Multiple(() => {

                Assert.That(response.IsSuccessStatusCode, Is.True);

                // What the EMSP answers is the whole configuration as it
                ...
                // In the client everything below this EMSP was handed,
                // and not in a copy of it.
                Assert.That(EMSP.DNSClient.UseCache,    Is.False);
                Assert.That(EMSP.DNSClient.MaxRetries,  Is.EqualTo(4));
        ...
        public async Task ANameServerInTheFormTheLogUsesIsRefusedAndNothingIsWritten()
        {
            ...
            var onDisk   = File.Exists(EMSP.ConfigFile.Path)
                               ? File.ReadAllText(EMSP.ConfigFile.Path)
                               : "";
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(EMSP.DNSClient.DNSServers.Select(server => server.ToString()),
        ...
        public async Task EmptyingTheNameServersIsRefused()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(EMSP.DNSClient.DNSServers.Any(), Is.True,
        ...
        /// for the parts of the EMSP that would have remembered to check
        /// a flag first.
        /// </summary>
        [Test]
        public async Task SwitchingNameResolutionOffTakesTheServersAway()
        {
            ...
            Assert.Multiple(() => {
                Assert.That(EMSP.DNSEnabled,                 Is.False);
                Assert.That(EMSP.DNSClient.DNSServers.Any(),  Is.False,
        ...
        public async Task PointingTheControllerAtAnotherTimeServerReplacesTheClient()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(response.IsSuccessStatusCode,              Is.True);
                Assert.That(EMSP.NTSClient.Hostname.ToString(),  Does.StartWith("ptbtime2.ptb.de"));

                Assert.That(EMSP.NTSClient,                      Is.Not.SameAs(before),
