...
namespace cloud.charging.open.EMSP.Tests
{
    ...
    /// EMSP's log has to ask it for the screen rather than write over that
    /// line. What is measured here is the EMSP's half of that: every entry the
    ...
    public class ConsoleLogTests
    {
        ...
        public void EveryEntryTheConsoleWouldShowIsHandedOver()
        {
            ...
            directory  = TestEMSPs.TemporaryDirectory("console-log");
            emsp       = TestEMSPs.New(directory, TestEMSPs.Offline, LogToConsole: true);

            var handed = new List<Action>();

            emsp.ShareConsoleWith(handed.Add);

            emsp.Log.Info ("Somebody is typing, and this has to wait its turn.", "test");
            emsp.Log.Debug("Below what the console shows, so nobody is asked.",  "test");
        ...
        /// <summary>
        /// An EMSP whose log does not reach the console has nothing to hand
        ...
        public void WithoutAConsoleThereIsNothingToShare()
        {
            ...
            directory  = TestEMSPs.TemporaryDirectory("console-log");
            emsp       = TestEMSPs.New(directory, TestEMSPs.Offline);

            var handed = 0;

            emsp.ShareConsoleWith(write => handed++);

            emsp.Log.Warning("Nobody is watching the console of this one.", "test");
