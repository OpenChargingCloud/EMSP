...
namespace cloud.charging.open.EMSP.Tests
{
    ...
    /// <summary>
    /// Whether an EMSP that was told to stop actually does.
    /// </summary>
    /// <remarks>
    ...
    /// itself stopped. An EMSP with one browser watching would therefore
    ...
    ///
    /// Each builds its own EMSP and stops it, so neither can use the one
    ...
    public class ShutdownTests
    {
        ...
        public void MakeADirectory()
        {
            ...
            directory = TestEMSPs.TemporaryDirectory("shutdown");
        ...
        public async Task StopsWithNobodyWatching()
        {
            ...
            var (EMSP, _) = await StartOne();

            var elapsed = await TimeTheStop(EMSP);

            Assert.That(elapsed, Is.LessThan(MustStopWithin),
                        $"An idle EMSP took {elapsed.TotalSeconds:F1} s to stop.");
        ...
        public async Task StopsWithABrowserOnTheLogsPage()
        {
            ...
            using (http)
            {
                ...
                // against an EMSP that cannot shut down at all.
                using var stream = await EventStream.OpenAndSettle(EMSP, http);

                // And now left alone, which is what a browser sitting on the
                // Logs page is when the EMSP is told to stop.
                var elapsed = await TimeTheStop(EMSP);

                Assert.That(elapsed, Is.LessThan(MustStopWithin),
                            $"An EMSP with one open event stream took {elapsed.TotalSeconds:F1} s to stop. " +
        ...
        /// pass the test above and hang a real EMSP.
        /// </summary>
        [Test]
        public async Task StopsWithSeveralBrowsersOnTheLogsPage()
        {

            var (EMSP, first) = await StartOne();
            ...
            try
            {
                ...
                streams.Add(await EventStream.OpenAndSettle(EMSP, first));

                for (var i = 0; i < 3; i++)
                {
                    var another = await SignIn(EMSP);
                    browsers.Add(another);
                    streams.Add(await EventStream.OpenAndSettle(EMSP, another));
                }

                var elapsed = await TimeTheStop(EMSP);

                Assert.That(elapsed, Is.LessThan(MustStopWithin),
                            $"An EMSP with {streams.Count} open event streams took {elapsed.TotalSeconds:F1} s to stop.");
        ...
        /// <summary>
        /// DisposeAsync stops as well, and an EMSP inside a using block
        ...
        /// <summary>
        /// An EMSP, listening, and a browser already signed in to it.
        /// </summary>
        private async Task<(EMSP EMSP, HttpClient HTTP)> StartOne()
        {

            // Offline: nothing here should wait on a network while it is
            // trying to measure how long stopping takes.
            var EMSP = TestEMSPs.New(directory, TestEMSPs.Offline);

            await EMSP.Start();

            return (EMSP, await SignIn(EMSP));

        }

        #endregion

        #region (private static) SignIn(EMSP)

        private static async Task<HttpClient> SignIn(EMSP EMSP)
        {

            var http = new HttpClient(new HttpClientHandler { UseCookies = true }) {
                           BaseAddress = new Uri(EMSP.WebInterfaceURL.ToString())
                       };

            // At the HTTPExt API: it is the only place that can check a
            // password, and the cookie it sets is what the JSON API reads.
            var response = await http.PostAsync(
                                     $"{EMSP.ExtAPIPath.ToString().TrimEnd('/')}/login",
                                     new FormUrlEncodedContent([
                                         new KeyValuePair<String, String>("login",     EMSP.DefaultAdminUser),
                                         new KeyValuePair<String, String>("password",  EMSP.GeneratedPassword ?? "")
                                     ])
                                 );
        ...
        #region (private static) TimeTheStop(EMSP)
        ...
        /// </remarks>
        private static async Task<TimeSpan> TimeTheStop(EMSP EMSP)
        {

            var clock    = Stopwatch.StartNew();

            var stopping = EMSP.Stop();
