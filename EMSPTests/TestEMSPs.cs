...
namespace cloud.charging.open.EMSP.Tests
{
    ...
    /// because not every test wants an EMSP that is started and taken away
    ...
    internal static class TestEMSPs
    {
        ...
        /// <param name="Configuration">What its configuration file says, or null for an EMSP nobody has configured.</param>
        ...
        public static EMSP New(String         Directory,
        ...
        /// </summary>
        /// <remarks>
        /// Written before an EMSP is built, because that is when it is
        ...
        /// constant, so that these tests do not fight with an EMSP
        ...
        public static String TemporaryDirectory(String Purpose)
            ...
            => Path.Combine(
                   Path.GetTempPath(),
                   $"emsp-{Purpose}-{Guid.NewGuid().ToString("N")[..12]}"
               );
