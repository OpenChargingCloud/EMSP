...
namespace cloud.charging.open.EMSP.Tests
{
    ...
    /// these tests also say whether the build embedded it - an EMSP whose
    /// frontend did not make it into the assembly answers every one of them
    /// with the JSON API and nothing else.
    /// </remarks>
    public class WebInterfaceTests : AEMSPTests
    {
        ...
        public async Task TheStubCarriesTheVersionOfTheController()
        {
            ...
            Assert.Multiple(() => {
                Assert.That(html.Contains($"content=\"v{EMSP.Version}\"", StringComparison.Ordinal), Is.True,
                            "The stub does not carry the version of the EMSP that served it.");
        ...
        public async Task TheStubReferencesABundleThatIsServed()
        {
            ...
            // and below "/EMSP" where several of these share one server, and
